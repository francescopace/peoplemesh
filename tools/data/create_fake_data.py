...
JSON_OUTPUT_ROOT = Path("tools/data/json/")
SQL_OUTPUT_DIR = Path("tools/data/sql/")
PMC_INGEST_BATCH_SIZE = 100
...
def build_group_records(survey: SurveyProfile, company_name: str, count: int, rng: random.Random) -> list[dict[str, Any]]:
    themes = [s.name for s in survey.skills] + survey.so_tags
    seen: set[str] = set()
    ...
    for idx in range(1, count + 1):
        theme = unique_themes[(idx - 1) % len(unique_themes)] if unique_themes else f"theme-{idx}"
        node_type = "COMMUNITY" if idx % 3 else "EVENT"
        gid = str(uuid.uuid5(UUID_NS, f"group:{idx}:{theme}"))
        out.append(
            {
                "id": gid,
                "node_type": node_type,
                "title": f"{theme} {'Guild' if node_type == 'COMMUNITY' else 'Summit'} #{idx:03d}",
                "description": f"{company_name} internal {node_type.lower()} focused on {theme}.",
                "tags": [theme.lower(), survey.skills[0].category if survey.skills else "tech", "community"],
...
def generate_embeddings_with_openai_compatible(
    users: list[dict[str, Any]],
    groups: list[dict[str, Any]],
    jobs: list[dict[str, Any]],
    base_url: str,
...
def write_seed_users_sql(path: Path, users: list[dict[str, Any]]) -> None:
    header = textwrap.dedent(
        f"""\
        -- Dev-only seed data generated by tools/data/create_fake_data.py.
        ...
        VALUES ({sql_quote(SYSTEM_OWNER_NODE_ID)}, 'USER', 'System: dev-seed-owner', 'System owner for synthetic dev seeds.', false, now(), now())
        ...
        WHERE node_type = 'USER' AND id <> {sql_quote(SYSTEM_OWNER_NODE_ID)};
        """
    )
    values: list[str] = []
    for u in users:
        values.append(
            "("
            f"{sql_quote(u['id'])}, 'USER', {sql_quote(u['title'])}, {sql_quote(u['description'])}, "
    ...
    consent_sql = textwrap.dedent(
        ...
        WHERE c.node_id = n.id
          ...
          AND n.id <> {sql_quote(SYSTEM_OWNER_NODE_ID)}
...
def write_seed_groups_sql(path: Path, groups: list[dict[str, Any]]) -> None:
    header = textwrap.dedent(
        ...
        DELETE FROM mesh.mesh_node
        WHERE node_type IN ('COMMUNITY', 'EVENT', 'INTEREST_GROUP');
        """
    )
    values: list[str] = []
    for g in groups:
        values.append(
            "("
            f"{sql_quote(g['id'])}, {sql_quote(g['node_type'])}, {sql_quote(g['title'])}, {sql_quote(g['description'])}, "
...
def write_seed_jobs_sql(path: Path, jobs: list[dict[str, Any]]) -> None:
    ...
    for j in jobs:
        ...
        values.append(
            "("
            f"'JOB', {sql_quote(j['title'])}, {sql_quote(j['description'])}, "
            f"{sql_text_array(j['skills_required'])}, {sql_jsonb(sd)}, {sql_quote(j['country'])}, true, {sql_embedding(j.get('embedding'))}, now(), now()"
            ")"
        )
    body = (
        "INSERT INTO mesh.mesh_node (node_type, title, description, tags, structured_data, country, searchable, embedding, created_at, updated_at)\nVALUES\n"
        + ",\n".join(values)
        + ";\n"
    )
    path.write_text(header + "\n" + body, encoding="utf-8")
...
def collect_skill_catalog() -> list[str]:
    """Read all SO survey tech columns across ALL respondents and return unique skill names."""
    seen: set[str] = set()
    items: list[str] = []

    with open(SO_SURVEY_CSV, encoding="utf-8") as f:
        for row in csv.DictReader(f):
            for col in SO_CATEGORY_MAP:
                val = row.get(col, "")
                if not val or val == "NA":
                    continue
                for item in val.split(";"):
                    item = item.strip()
                    if not item or item.startswith("Other"):
                        continue
...
def write_seed_skill_catalog_sql(path: Path, skills: list[str]) -> None:
    header = textwrap.dedent(
        f"""\
        -- Dev-only SO skill dictionary seed generated by tools/data/create_fake_data.py.
        -- Source: Stack Overflow Developer Survey 2025
    ...
    for raw_name in skills:
        ...
        alias_array = sql_text_array([raw_name]) if raw_name != normalized else sql_text_array([])
        values.append(
            f"    ({sql_quote(normalized)}, {alias_array}, 0)"
        )
    body = (
...
def pmc_json_source() -> str:
    return "create_fake_data"


def normalize_pmc_work_mode(value: str | None) -> str | None:
    if value is None:
        return None
...
def build_pmc_users_payload(users: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
        ...
        payload_users.append(
            {
                ...
                "tags": list(user["tags"]),
                "structured_data": user["structured_data"],
                "searchable": True,
            }
        )
    return {"users": payload_users}


def build_pmc_nodes_payload(groups: list[dict[str, Any]], jobs: list[dict[str, Any]]) -> dict[str, list[dict[str, Any]]]:
        ...
        payload_nodes.append(
            {
                ...
                "source": pmc_json_source(),
                "external_id": f"synthetic-{group['node_type'].lower()}-{group['id']}",
                "title": group["title"],
                "description": group["description"],
                "country": group["country"],
                "tags": list(group["tags"]),
                "searchable": True,
                "structured_data": group["structured_data"],
            }
        )
        ...
        payload_nodes.append(
            {
                ...
                "country": job["country"],
                "tags": list(job["skills_required"]),
                "searchable": True,
                "structured_data": {
                    "generated_by": pmc_json_source(),
                },
                "requirements_text": job["requirements_text"],
                "skills_required": list(job["skills_required"]),
                "work_mode": normalize_pmc_work_mode(job.get("work_mode")),
                "employment_type": str(job["employment_type"]).upper(),
...
def write_json_file(path: Path, payload: dict[str, list[dict[str, Any]]]) -> None:
    path.write_text(json.dumps(payload, indent=2, ensure_ascii=True) + "\n", encoding="utf-8")


def clear_existing_pmc_json_files(output_dir: Path) -> None:
    for pattern in ("users-*.json", "nodes-*.json"):
        for path in output_dir.glob(pattern):
            path.unlink()


def write_pmc_json_payloads(
    output_dir: Path,
    users: list[dict[str, Any]],
    groups: list[dict[str, Any]],
    jobs: list[dict[str, Any]],
) -> tuple[int, int]:
    users_payload = build_pmc_users_payload(users)
    nodes_payload = build_pmc_nodes_payload(groups, jobs)

    user_batches = chunked(users_payload["users"], PMC_INGEST_BATCH_SIZE)
    node_batches = chunked(nodes_payload["nodes"], PMC_INGEST_BATCH_SIZE)

    for idx, batch in enumerate(user_batches, start=1):
        write_json_file(output_dir / f"users-{idx:03d}.json", {"users": batch})

    for idx, batch in enumerate(node_batches, start=1):
        write_json_file(output_dir / f"nodes-{idx:03d}.json", {"nodes": batch})

    return len(user_batches), len(node_batches)


def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Generate public-source company data focused by company type and write local dev seed SQL files or PMC ingest JSON payloads."
    )
    ...
    parser.add_argument(
        ...
        help=f"OpenAI-compatible embeddings base URL (default: {OPENAI_BASE_URL})",
    )
    parser.add_argument(
        "--api-key",
        default="",
        help="API key for the embeddings endpoint (defaults to OPENAI_API_KEY environment variable when set)",
    )
    parser.add_argument(
        "--embedding-model",
    ...
    parser.add_argument(
        "--json",
        action="store_true",
        help="Write PMC ingest JSON payload batches instead of SQL seed files. Embeddings are skipped.",
    )
...
def main() -> int:
    ...
    base_url = args.base_url.strip() or OPENAI_BASE_URL
    embedding_model = args.embedding_model.strip() or EMBEDDING_MODEL
    ...
    print("[2/3] Building company data from SO survey profile..." if args.json else "[2/5] Building company data from SO survey profile...")
    phase_start = perf_counter()
    ...
    embedding_model_name = "skipped (--json)" if args.json else ""
    write_groups_seconds = 0.0
    write_jobs_seconds = 0.0
    write_catalog_seconds = 0.0
    write_users_seconds = 0.0
    write_seconds = 0.0
    ...
    if args.json:
        print("[2/3] Build completed.")
        json_output = workspace / JSON_OUTPUT_ROOT 
        json_output.mkdir(parents=True, exist_ok=True)
        clear_existing_pmc_json_files(json_output)

        print(f"[3/3] Writing PMC ingest JSON files in batches of {PMC_INGEST_BATCH_SIZE}...")
        phase_start = perf_counter()
        write_users_start = perf_counter()
        user_batch_files, _ = write_pmc_json_payloads(json_output, users, [], [])
        write_users_seconds = perf_counter() - write_users_start
        write_nodes_start = perf_counter()
        _, node_batch_files = write_pmc_json_payloads(json_output, [], groups, jobs)
        write_groups_seconds = perf_counter() - write_nodes_start
        write_seconds = perf_counter() - phase_start
        output_dir = json_output
        print("[done] JSON generation completed.")
    else:
        ...
        phase_start = perf_counter()
        generate_embeddings_with_openai_compatible(
            users=users,
            groups=groups,
            jobs=jobs,
            base_url=base_url,
        ...
        sql_output = workspace / SQL_OUTPUT_DIR
        sql_output.mkdir(parents=True, exist_ok=True)

        print("[4/5] Writing SQL seed files...")
        phase_start = perf_counter()
        write_users_start = perf_counter()
        write_seed_users_sql(sql_output / "R__dev_seed_users.sql", users)
        write_users_seconds = perf_counter() - write_users_start
        write_groups_start = perf_counter()
        write_seed_groups_sql(sql_output / "R__dev_seed_groups.sql", groups)
        write_groups_seconds = perf_counter() - write_groups_start
        write_jobs_start = perf_counter()
        write_seed_jobs_sql(sql_output / "R__dev_seed_jobs.sql", jobs)
        ...
        write_seed_skill_catalog_sql(sql_output / "R__dev_seed_skill_catalog_so.sql", skill_catalog)
    ...
    if args.json:
        print("- output_format: pmc-json")
        print(f"- batch_size_limit: {PMC_INGEST_BATCH_SIZE}")
        print(f"- user_batch_files: {user_batch_files}")
        print(f"- node_batch_files: {node_batch_files}")
    else:
        print(f"- skill_dictionary: {len(skill_catalog)}")
    ...
    print(f"- output_dir: {output_dir.relative_to(workspace)}")
    print("- timings:")
    ...
    if args.json:
        print(f"  - write_pmc_json_total: {fmt_seconds(write_seconds)}")
        print(f"  - write_users_json: {fmt_seconds(write_users_seconds)}")
        print(f"  - write_nodes_json: {fmt_seconds(write_groups_seconds)}")
    else:
        ...
        print(f"  - write_seed_skill_dictionary_sql: {fmt_seconds(write_catalog_seconds)}")
