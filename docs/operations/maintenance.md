...
Endpoint contracts (paths, payload shapes, and parameter constraints) are maintained in [`../reference/api.md`](../reference/api.md).
...
The script requires `bash` and `curl`. For formatted JSON output, it uses `jq` when available (falls back to `python3 -m json.tool`).

Then run commands with the executable (omit `--base-url` for the local default):
