...
Generate realistic dev seed data for local development and demos using the Stack Overflow Developer Survey 2025 as backbone.
...
| Skill catalog (263 technologies in 10 categories) | SO survey (all respondents) | `R__dev_seed_skill_catalog_so.sql` |
...
```bash
python3 tools/create_fake_data.py
```

This uses the defaults: `--company-type it`, `--company-name "Acme Corp"`, 500 users, 50 jobs, 100 groups.
