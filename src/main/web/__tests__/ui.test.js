...
describe("toastForPromise()", () => {
  ...
  it("uses shared user-facing fallback when no custom error handler is provided", async () => {
    const err = Object.assign(new Error("Gateway timeout"), { status: 502 });
    await expect(
      toastForPromise(Promise.reject(err), {
        loadingMessage: "Loading",
        minVisibleMs: 0,
    ...
    const toasts = [...document.querySelectorAll(".toast-container .toast")].map((node) => node.textContent);
    expect(toasts).toContain("Request failed");
  });
});
