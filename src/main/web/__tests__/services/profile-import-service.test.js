...
import { buildFieldMap, buildPartialProfile } from "../../assets/js/services/profile-import-service.js";
...
describe("profile-import-service", () => {
  it("buildFieldMap marks mergeable array conflicts", () => {
    ...
    const field = buildFieldMap(imported, current)
  ...
  it("buildPartialProfile merge clamps learning_areas to max 50", () => {
    const currentTopics = Array.from({ length: 30 }, (_, i) => `current-${i}`);
    const importedTopics = Array.from({ length: 30 }, (_, i) => `imported-${i}`);
    const imported = {
      interests_professional: { learning_areas: importedTopics },
    };
    const current = {
      interests_professional: { learning_areas: currentTopics },
    };
    const selected = new Set(["interests.learning_areas"]);
    const mergeModes = new Map([["interests.learning_areas", "merge"]]);

    const result = buildPartialProfile(imported, current, selected, mergeModes);

    expect(result.interests_professional.learning_areas).toHaveLength(50);
    expect(result.interests_professional.learning_areas[0]).toBe("current-0");
    expect(result.interests_professional.learning_areas[49]).toBe("imported-19");
  });
});
