...
public class NodeEmbeddingMaintenanceService {
    ...
    private void processBatch(List<UUID> batchIds, MutableJobState job) {
        try {
            ...
            LOG.warnf("Batch embedding failed, fallback to single-item mode: %s", e.getMessage());
            processBatchSingleFallback(batchIds, job);
        }
    }
    ...
    private void processBatchSingleFallback(List<UUID> batchIds, MutableJobState job) {
        for (UUID nodeId : batchIds) {
            try {
                regenerateSingleNodeEmbedding(nodeId);
                job.recordProgress(1, 1, 0);
            } catch (Exception e) {
                job.recordProgress(1, 0, 1);
