...
class NodeEmbeddingMaintenanceServiceTest {
    ...
    @Test
    void startRegenerationEmbeddings_whenBatchFails_marksJobFailed() throws InterruptedException {
        UUID actorId = UUID.randomUUID();
        ...
        assertEquals(NodeEmbeddingMaintenanceService.JobState.FAILED, finalStatus.status());
        assertEquals(2, finalStatus.total());
        assertEquals(0, finalStatus.processed());
        assertEquals(0, finalStatus.succeeded());
        assertEquals(0, finalStatus.failed());
        assertNotNull(finalStatus.error());
