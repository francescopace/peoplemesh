...
class MatchingUtilsTest {
    ...
    @Test
    void parseArray_sqlArray_exception_throws() {
        java.sql.Array sqlArray = new StubSqlArray(null);
        assertThrows(IllegalStateException.class, () -> SqlParsingUtils.parseArray(sqlArray));
    }
    ...
    void parseEnum_invalid_throws() {
        assertThrows(IllegalArgumentException.class, () -> SqlParsingUtils.parseEnum(Color.class, "PURPLE"));
    }
    ...
    @Test
    void jaccardSimilarity_whenFuzzyMatchCollapsesUnion_keepsFiniteScore() {
        double sim = MatchingUtils.jaccardSimilarity(List.of("java se"), List.of("java"));
        assertTrue(sim >= 0.0 && sim <= 1.0);
    }
    ...
    @Test
    void intersectCaseInsensitive_ignoresBlankNormalizedTokens() {
        List<String> result = MatchingUtils.intersectCaseInsensitive(List.of("+++", "Java"), List.of("java"));
    ...
    void structuredWorkMode_invalidWorkMode_returnsNull() {
        ...
        assertThrows(IllegalArgumentException.class, () -> MatchingUtils.structuredWorkMode(node));
    }
    ...
    void structuredEmploymentType_invalidType_returnsNull() {
        ...
        assertThrows(IllegalArgumentException.class, () -> MatchingUtils.structuredEmploymentType(node));
    }

    @Test
    void termsMatch_handlesShortSingleTokenAndExclusivePairs() {
        assertFalse(MatchingUtils.termsMatch("Go", "Golang"), "single short tokens should not fuzzy-match");
        assertFalse(MatchingUtils.termsMatch("Java", "JavaScript"), "exclusive token pairs should not match");
        assertTrue(MatchingUtils.termsMatch("Data Engineering", "engineering data"));
    }

    @Test
    void computeLevelAwareCoverage_handlesNullMinLevelAndProportionalLevels() {
        List<SkillWithLevel> required = List.of(
                new SkillWithLevel("Java", null),
                new SkillWithLevel("Kotlin", 4)
        );
        Map<String, Short> levels = Map.of("java", (short) 1, "kotlin", (short) 2);
        double coverage = MatchingUtils.computeLevelAwareCoverage(required, List.of("Java", "Kotlin"), levels);
        assertTrue(coverage > 0.5 && coverage < 1.0);
    }
