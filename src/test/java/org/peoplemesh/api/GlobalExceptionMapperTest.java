...
class GlobalExceptionMapperTest {
    ...
    void securityException_returns403() {
        Response response = mapper.toResponse(new SecurityException("forbidden"));
        assertEquals(403, response.getStatus());
    }

    @Test
    void rateLimitException_returns429_withRetryAfter() {
        Response response = mapper.toResponse(new RateLimitException("rate limited"));
    ...
    void illegalStateException_returns409() {
        Response response = mapper.toResponse(new IllegalStateException("conflict"));
